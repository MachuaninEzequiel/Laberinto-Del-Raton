...
def main():
    ...
    mouse_col, mouse_row = LABYRINTH_WIDTH - 1, LABYRINTH_HEIGHT - 1  # Inicialmente en la esquina inferior izquierda
    ...
    while running:
        ...
        if is_mouse_moving:
            ...
            neighbors = [(mouse_row - 1, mouse_col), (mouse_row + 1, mouse_col),
                         (mouse_row, mouse_col - 1), (mouse_row, mouse_col + 1)]
            valid_neighbors = [(row, col) for row, col in neighbors
                               if 0 <= row < LABYRINTH_HEIGHT and 0 <= col < LABYRINTH_WIDTH
                               and not check_wall_collision(grid, row, col)]
            if valid_neighbors:
                mouse_row, mouse_col = random.choice(valid_neighbors)
        ...
        clock.tick(100)  # Limitar la velocidad de actualización
